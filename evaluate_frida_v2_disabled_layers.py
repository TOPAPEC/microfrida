...
class FridaModel:
    ...
    def __init__(self, model_name="ai-forever/FRIDA"):
        ...
        original_model = T5EncoderModel.from_pretrained(model_name)
        
        print("Original layer count:", len(original_model.encoder.block))
        
        self.model = copy.deepcopy(original_model)
        
        layers_to_keep = [0, 2, 4, 6, 8, 10, 12] + list(range(14, len(self.model.encoder.block)))
        new_blocks = torch.nn.ModuleList([
            self.model.encoder.block[i] for i in layers_to_keep
        ])
        self.model.encoder.block = new_blocks
        
        print("Modified layer count:", len(self.model.encoder.block))
        print(f"Kept layers: {layers_to_keep}")
    ...
    def plot_layer_statistics(self, stats: Dict[str, List[float]], save_path: Optional[str] = None):
        ...
        plt.title('Layer-wise Activation Statistics (Disabled Layers 2-13)')
        plt.legend()
        plt.grid(True)
...
def main():
    ...
    results = evaluation.run(model, output_folder="results_disabled_layers")
    ...
    model.plot_layer_statistics(stats, save_path="layer_activations_disabled.png")
    print("\nLayer activation plot saved as layer_activations_disabled.png")
