...
def load_training_dataset(tokenizer, limit=50000, cache_file="tokenized_cache.parquet", local_rank=-1):
    ...
    if local_rank <= 0:
        if os.path.exists(cache_file):
            ...
            ds = ds.map(preprocess, batched=False, num_proc=32, desc="Tokenizing dataset")
    ...
    if dist.is_initialized():
        ...
        dist.barrier()
    logger.info(f"Rank {local_rank}: Loading tokenized dataset from cache...")
    ds = load_dataset("parquet", data_files={"train": cache_file}, cache_dir="./hf_datasets")["train"]
    ds.set_format(type="torch", columns=["input_ids", "attention_mask"])
    return ds
